...
async function displayDoctors() {
  const healthcareCollection = collection(db, "healthcare_providers");
  ...
  try {
      ...
      querySnapshot.forEach((doc) => {
          ...
          row.innerHTML = `
              ...
              <td>${doctorData.location}</td>
              <td>${doctorData.speciality}</td>
              <td>${doctorData.availability}</td>
              <td>
                  <a href="admin_doctorsUpdate.html?id=${doc.id}" class="btn"><i class="fas fa-edit"></i> Edit</a>
                  <a href="admin_doctors.html?id=${doc.id}" class="btn remove-btn" data-patient-id="${doc.id}"><i class="fas fa-trash"></i> Remove</a>
                  <a href="admin_doctorsView.html?id=${doc.id}" class="btn"><i class="fas fa-eye"></i> View</a>
              </td>
          `;
...
const healthCareID = urlParams.has('id') ? urlParams.get('id') : null;
...
if (healthCareID !== null) {
  ...
  .then(() => {
    console.log('Healthcare Provider removed from Firestore.');
