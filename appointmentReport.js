...
import {getFirestore, doc, collection, query, where,  getDocs, getDoc, addDoc } from  "https://www.gstatic.com/firebasejs/10.1.0/firebase-firestore.js";
...
async function retrieveAppointment() {
  ...
  if (user) {
      const uid = user.uid;
      const userRef = doc(db, "appointments", uid); // Correct variable name
    ...
    try {
      ...
      if (docSnapshot.exists()) {
        ...
        if (userData) {
          ...
          document.getElementById("doc_name").value = userData.doctorName;
          document.getElementById("doc_speciality").value = userData.doctorSpeciality;
          document.getElementById("appointment_type").value = userData.appointmentType;
          document.getElementById("cell").value = userData.cell;
