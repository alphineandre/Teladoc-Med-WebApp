...
async function displayMedicalReport() {
  const medicalReportCollection = collection(db, "healthcare_providers", auth.currentUser.uid, "medical_records");

  try {
      const querySnapshot = await getDocs(medicalReportCollection);
      const medicalReportList = document.getElementById("medicalRecords-list");

      querySnapshot.forEach((doc) => {
          const medicalRportData = doc.data();

          const row = document.createElement("tr");
          row.innerHTML = `
              <td>${medicalRportData.patientName}</td>
              <td>${medicalRportData.phoneNumber}</td>
              <td>${medicalRportData.age}</td>
              <td>${medicalRportData.email}</td>
              <td>${medicalRportData.physicalAddress}</td>
              <td>${medicalRportData.martial_Status}</td>
              <td>${medicalRportData.Appointment_Date}</td>
              <td>${medicalRportData.appointmentType}</td>
              <td>${medicalRportData.Symptoms}</td>
              <td>${medicalRportData.nextAppointment}</td>
              <td>${medicalRportData.createdAt}</td>
              
              <td>
              <a href="doc_medicalRecordUpdate.html?id=${doc.id}" class="btn"><i class="fas fa-edit"></i> Edit</a>
              <a href="doc_medicalRecordView.html?id=${doc.id}" class="btn"><i class="fas fa-eye"></i> View</a>
          </td>
      `;

      medicalReportList.appendChild(row);
      });
  } catch (error) {
